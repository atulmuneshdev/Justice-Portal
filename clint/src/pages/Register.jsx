...
const Register = () => {
    ...
    const validateEnrollment = (enrollment) => {
        const pattern = /^[A-Z]{2}\/\d{1,6}\/\d{4}$/;
        return pattern.test(enrollment);
    };

    const handleEnrollmentChange = (e) => {
        const value = e.target.value.toUpperCase(); // auto uppercase
        setFormData({ ...formData, enrollment: value });

        if (value === "") {
            setError("");
        } else if (!validateEnrollment(value)) {
            setError("Format: UP/1234/2020");
        } else {
            setError("");
        }
    };
                ...
                <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
                    <div className="space-y-4">
                        <div className="w-full flex flex-col sm:flex-row gap-4">
                            ...
                            {/* Enrollment */}
                            { role === 'advocate' && (<div className="w-full">
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider ml-1">
                                    Enrollment
                                </label>
                                <div className="relative mt-1">
